...
DB_PORT     = int(os.environ.get("DB_PORT") or "3306")  # or защищает от пустой строки
...
async def _get_db_pool():
    ...
    try:
        ...
        pool = await aiomysql.create_pool(
            ...
            ssl=ssl_ctx,       # SSLContext — правильный тип для aiomysql!
            charset="utf8mb4",
            autocommit=True,
            connect_timeout=10,
            minsize=1,
...
async def _init_db_pool():
    ...
    Вызывается из _startup() — уже внутри event loop бота.
    ...
    # Загружаем данные из MySQL в глобальные переменные
...
# затем заполняются из MySQL в _startup() (уже внутри event loop бота)
DATABASE    = {}
ECONOMY     = {}
PUNISHMENTS = {}
STAFF       = {}
