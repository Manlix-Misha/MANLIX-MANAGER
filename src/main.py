...
def extract_id(text):
    ...
    # Регулярка для поиска ID в ссылках и упоминаниях
...
async def check_mute(message: Message):
    """Проверка мута перед выполнением любой команды"""
...
async def help_handler(message: Message):
    if await check_mute(message): return
...
# --- 5. КОМАНДЫ ПОЛЬЗОВАТЕЛЕЙ (ТОЛЬКО 3) ---
...
async def mute_handler(message: Message, args=None):
    ...
    await message.answer(f"Администратор {mod_nick} выдал блокировку чата пользователю vk.com/id{target_id} на {minutes} минут.")

# --- 7. КОМАНДЫ РУКОВОДСТВА ---
...
# --- 8. ГЛОБАЛЬНЫЙ УДАЛЯТОР ДЛЯ ЗАМУЧЕННЫХ (ДЛЯ ОБЫЧНЫХ СООБЩЕНИЙ) ---
@bot.on.message()
async def auto_delete_muted(message: Message):
    await check_mute(message)
