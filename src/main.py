...
async def check_active(m: Message):
    ...
    if str(m.from_id) in DATABASE.get("gbans", []): return False
    return str(m.peer_id) in DATABASE.get("chats", {})
...
# --- 5. КОМАНДЫ ПОЛЬЗОВАТЕЛЕЙ И МОДЕРАЦИИ ---
...
async def mute_cmd(m: Message, args=None):
    ...
    kb = (Keyboard(inline=True)
          .add(Text("Снять мут", {"cmd": "unmute_edit", "user": t}), color=KeyboardButtonColor.POSITIVE)
          .row()
          .add(Text("Очистить", {"cmd": "clear"}), color=KeyboardButtonColor.NEGATIVE)).get_json()
    
    msg = f"[id{m.from_id}|Модератор MANLIX] выдал(-а) мут [id{t}|пользователю]\nПричина: {reason}\nМут выдан до: {end.strftime('%d/%m/%Y %H:%M:%S')}"
    await m.answer(msg, keyboard=kb)
...
async def setnick_cmd(m: Message, args=None):
    ...
    pid = str(m.peer_id)
    r = get_user_data(m.peer_id, t)[0] # Сохраняем текущую роль (Фикс бага №1)
...
async def grant_role(m: Message, args, req_rank, role_name, action_text):
    ...
    if not t: return
    pid = str(m.peer_id)
    _, target_nick = get_user_data(m.peer_id, t) # Сохраняем текущий ник (Фикс бага №1)
    if "chats" not in DATABASE: DATABASE["chats"] = {}
    if pid not in DATABASE["chats"]: DATABASE["chats"][pid] = {"staff": {}}
...
# --- 8. СИСТЕМНЫЕ КОМАНДЫ (Спец. Руководитель и Замы) ---
...
async def gban_cmd(m: Message, args=None):
    ...
    t = m.reply_message.from_id if m.reply_message else extract_id(args)
    if t:
        if str(t) not in DATABASE["gbans"]: DATABASE["gbans"].append(str(t))
        await push_to_github(DATABASE, GH_PATH_DB, EXTERNAL_DB, "GBan")
        await m.answer(f"Пользователь id{t} заблокирован во всех беседах.")

@bot.on.message(text=["/gunbanpl", "/gunbanpl <args>"])
async def gunban_cmd(m: Message, args=None):
    if not await check_access(m, "Зам. Специального Руководителя"): return
    t = m.reply_message.from_id if m.reply_message else extract_id(args)
    if t and str(t) in DATABASE["gbans"]:
        DATABASE["gbans"].remove(str(t))
        await push_to_github(DATABASE, GH_PATH_DB, EXTERNAL_DB, "GUnban")
        await m.answer(f"Пользователь id{t} разблокирован.")
