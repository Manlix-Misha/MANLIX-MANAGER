...
async def all_buttons(event: MessageEvent):
    ...
    if cmd in ("unmute_btn", "clear_msg"):
        ...
        if cmd == "unmute_btn":
            ...
            new_text = f"[id{actor_id}|{a_name2}] очистил(-а) сообщения [id{uid}|{u_name2}]"
            try:
                await bot.api.request("messages.edit", {
                    "peer_id": peer_id,
                    "message": new_text,
...
async def get_info_cmd(m: Message, args=None):
    ...
    for chat_pid, chat_data in DATABASE.get("chats", {}).items():
        ...
        if uid in staff:
            user_entry = staff[uid]
            roles = [user_entry[0]] if user_entry else []
            if len(user_entry) > 2 and isinstance(user_entry[2], list):
                roles.extend(user_entry[2])
            if "Владелец" in roles:
                owner_count += 1
...
async def get_game_cmd(m: Message, args=None):
    ...
    await m.answer(
        ...
        f"| Выиграно в дуэлях: « {duel_wins_sum}$ »\n"
        f"| Проиграно в дуэлях: « {duel_losses_sum}$ »\n\n"
        f"| Время: {now.strftime('%H:%M:%S')}\n"
        f"| Дата: {now.strftime('%d/%m/%Y')}"
    )
